        ...
        <div className="relative min-h-screen bg-white text-[#2D6A4F] font-body">
            ...
            <section className="py-24 bg-white relative z-10 overflow-hidden">
                <div className="container mx-auto px-4 md:px-6 max-w-7xl">
                    <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
                        <div className="order-2 lg:order-1 flex-1 w-full max-w-2xl mx-auto flex flex-col justify-center">
                            <motion.div
                                initial={{ opacity: 0, y: 20 }}
                        ...
                        <div className="order-1 lg:order-2 relative w-full flex-1">
                            <div className="relative overflow-hidden group rounded-[2.2rem]">
                                ...
                                />
                                <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent rounded-[2.2rem] opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section className="py-24 bg-brand-light-bg relative z-10 overflow-hidden">
                <div className="container mx-auto px-4 md:px-6 max-w-7xl">
                    ...
                    <div ref={modesRef} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {MODES.map((mode) => (
                            <SpotlightCard
                                key={mode.tag}
                                className={`mode-card group relative overflow-hidden rounded-[2rem] border p-8 md:p-12 transition-all duration-300 ${mode.bgColor} ${mode.border} ${mode.hoverShadow} ${mode.hoverBorder} hover:-translate-y-2`}
                                spotlightColor={mode.spotlight}
                            >
                                <div className="relative z-10 h-full flex flex-col">
                                    <div className="flex justify-between items-start mb-8">
                                        <div className={`w-16 h-16 rounded-2xl border flex items-center justify-center flex-shrink-0 bg-white/40 transition-transform duration-300 group-hover:scale-110 shadow-sm ${mode.iconBorder}`}>
                                            <img src={mode.icon} alt={mode.title} className="w-8 h-8 object-contain" />
                                        </div>
                                        <div className={`px-4 py-2 rounded-full ${mode.tagBg} ${mode.tagText} text-xs font-bold tracking-widest uppercase border ${mode.tagBorder} shadow-sm`}>
                                            {mode.tag}
                                        </div>
                                    </div>
                                    <h3 className={`font-serif text-3xl font-bold mb-4 ${mode.headingColor}`}>{mode.title}</h3>
            ...
            <section className="py-24 bg-white relative z-10 overflow-hidden">
                <div className="container mx-auto px-4 md:px-6 max-w-7xl">
                    ...
                    <div ref={industriesRef} className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
                        {INDUSTRIES.map((ind, idx) => (
                            ...
                            >
                                <div className="relative z-10 flex items-center gap-6">
                                    <div className={`w-14 h-14 rounded-2xl border flex items-center justify-center flex-shrink-0 bg-white/40 ${ind.iconBorder} transition-transform duration-300 group-hover:scale-110 shadow-sm`}>
                ...
                <div className="container mx-auto px-4 md:px-6 max-w-4xl">
                    ...
                    <div ref={techRef} className="flex flex-wrap justify-center gap-4">
                        {TECH_STACK_MARQUEE.map((tech, index) => (
                            <div key={index} className="tech-badge flex flex-col items-center justify-center bg-white border border-gray-100 rounded-3xl p-6 w-36 h-36 shadow-sm hover:shadow-md transition-shadow">
                                <div className="h-12 w-full flex items-center justify-center px-2 mb-3">
                                ...
                                <div className="flex flex-col items-center text-center">
                                    <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">{tech.category}</span>
                                    <span className="text-xs font-medium text-gray-700 leading-tight">{tech.name}</span>
                                </div>
                            </div>
                        ))}
                    </div>
